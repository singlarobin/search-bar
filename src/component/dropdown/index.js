...
const DropDown = (props) => {
  ...
  const handleButtonClick = () =>
    alert('Button functionality not Implemented!!');
      ...
      {isEmptyString(errorMessage) ? (
        <div className={classes.container}>
          <div className={classes.leftContainer}>
            {!isEmptyObject(suggestions) && suggestions.length !== 0 ? (
              <Fragment>
            ...
            {!isEmptyObject(collections) && collections.length !== 0 ? (
              <Fragment>
          ...
          <div className={classes.rightContainer}>
            {!isEmptyObject(results) && results.length !== 0 ? (
              <Fragment>
                <div className={classes.productHeading}>
                  POPULAR PRODUCTS IN ' {product} '{' '}
                </div>
                <div className={classes.productList}>
                  {results.map((item, index) => (
                    <Product
                      key={index}
                      name={
                        !isEmptyString(item.collectionname)
                          ? item.collectionname
                          : item.productname
                      }
                      mrpPrice={item.mrpprice}
                      withoutDiscountPrice={item.pricewithoutdiscount}
